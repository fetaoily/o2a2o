...
import { KeyPoolRegistry } from "./forwarder";
...
// keys nor a provider-wide key has no pool (poolFor throws) and appears as {}
// so a misconfiguration stays visible in the inventory.
export function healthKeysBody(cfg: AppConfig): Record<string, unknown> {
  ...
  for (const model of cfg.models) {
    ...
    catch { models[model.name] = {}; continue; } // keyless + globalless: no pool
    pool.maybeRecover();
