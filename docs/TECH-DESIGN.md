...
- **§7.3 延迟记账口径（live-test hardening Task 3）**：非流式 `forwardWithFailover` 每次尝试于 fetch 前取 t0，2xx 响应头到达后 `pool.recordSuccess(keyId, Date.now() - t0)`——响应头口径，与流式 first_packet 同约定；该喂入自 M3（498fdbb）即已存在，实测发现 3「非流式成功不喂延迟样本」在主干不成立（实测观察最可能是动态 Key 路径按设计零记账所致）。Task 3 以测试钉死该口径防回归：集成（~30ms 延迟 mock 上游 → `/health/keys` `avgLatency > 0`）+ 单测（采样值 ≥ 注入延迟且 < timeoutMs，防常量 0 / 超时值伪造；变异验证：喂 0 时两测均红）。动态 Key 路径零记账维持现状。
- **§5/§6.2 上游格式覆盖（实测发现 4，每模型 `upstream_format: "chat"`）**：实测发现 4——只服务 `/v1/chat/completions` 的 openai 系上游（智谱 `/api/paas/v4`，其 `/v1/responses` 返回 404）无法承接 `/v1/responses` 入站的同 provider 透传。裁定：`models[].upstream_format` 为可选字段，仅接受 `"chat"` 且仅可设于 `provider: "openai"` 模型（anthropic 上设置或取 `"responses"` 等其他值 → 加载期 `ConfigError` 带字段路径；responses 请求编码器不存在，YAGNI）。设置后 `resolveRoute` 的「同 provider 透传」分支对该模型的 responses 入站不生效：走 `responsesToIr` → `irToChat` 既有转换，endpoint 固定 `/v1/chat/completions`；`nativeFormatOf` 同步按 `openai_chat`（响应解码与流式 srcFormat 单点跟随，chat-SSE → responses-SSE 方向 M2 流式枢纽已有）。缺省（未设置）行为逐字节不变；与 `base_url` 正交可叠加（合称即智谱真实形态）。
